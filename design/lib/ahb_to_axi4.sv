...
(
   ...
   output logic [pt.XLEN-1:0]             axi_awaddr,
   ...
   output logic [pt.XLEN-1:0]             axi_araddr,
   ...
   // AHB-Lite signals
   input logic [pt.XLEN-1:0]              ahb_haddr,     // ahb bus address
...
// FSM to control the bus states and when to block the hready and load the command buffer
   ...
   rvdff_fpga #(.WIDTH(pt.XLEN)) haddr_ff  (.din(ahb_haddr[pt.XLEN-1:0]),    .dout(ahb_haddr_q[pt.XLEN-1:0]), .clk(ahb_addr_clk), .clken(ahb_addr_clk_en), .rawclk(clk), .*);
   ...
   assign axi_awaddr[pt.XLEN-1:0]            = cmdbuf_addr[pt.XLEN-1:0];
  ...
  // AXI Write Response - Always ready. AHB does not require a write response.
   ...
   assign axi_araddr[pt.XLEN-1:0]            = cmdbuf_addr[pt.XLEN-1:0];
