...
`include "eh2_param.vh"
   ...
   input logic                          ldst_samebeat_dc5,                  // Hi/Lo are within same dw
   ...
   input logic [pt.BUS_WIDTH-1:0]       bus_rsp_rdata,
   ...
   input logic [pt.BUS_WIDTH-1:0]       bus_rsp_rdata_q,
   ...
   output logic [pt.XLEN-1:0]           lsu_imprecise_error_addr_any,     // address of the imprecise error
   ...
   logic   [DEPTH-1:0][pt.XLEN-1:0]     buf_data;
   ...
   rvdffs  #(.WIDTH(1))              ibuf_samedwff     (.din(ldst_samebeat_dc5),            .dout(ibuf_samedw),     .en(ibuf_wr_en),                   .clk(lsu_bus_ibuf_c1_clk), .*);
   ...
   assign obuf_addr_in[pt.XLEN-1:0] = ibuf_buf_byp ? lsu_addr_dc5[pt.XLEN-1:0] : buf_addr[CmdPtr0];
   assign obuf_sz_in[1:0] = ibuf_buf_byp ? ({lsu_pkt_dc5.word, lsu_pkt_dc5.half} |
                                            ({2{(pt.XLEN == 64) & lsu_pkt_dc5.dword}}))
                                         : buf_sz[CmdPtr0];
   assign obuf_aligned_in = ibuf_buf_byp ? is_aligned_dc5 : ((obuf_sz_in[1:0] == 2'b00)                          |
                                                            ((obuf_sz_in[1:0] == 2'b01) & ~obuf_addr_in[0])      |
                                                            ((obuf_sz_in[1:0] == 2'b10) & ~(|obuf_addr_in[1:0])) |
                                                            ((obuf_sz_in[1:0] == 2'b11) & ~(|obuf_addr_in[2:0]) & (pt.XLEN == 64)));
   ...
   assign obuf_merge_en = ((CmdPtr0 != CmdPtr1) & found_cmdptr0 & found_cmdptr1 & (buf_state[CmdPtr0] == CMD) & (buf_state[CmdPtr1] == CMD) &
                           ...
                           ((buf_write[CmdPtr0] & buf_write[CmdPtr1] & (buf_addr[CmdPtr0][pt.XLEN-1:3] == buf_addr[CmdPtr1][pt.XLEN-1:3]) & ~bus_coalescing_disable & !pt.BUILD_AXI_NATIVE) |
                            (~buf_write[CmdPtr0] & buf_dual[CmdPtr0] & ~buf_dualhi[CmdPtr0] & buf_samedw[CmdPtr0]))) |  // CmdPtr0/CmdPtr1 are for same load which is within a DW
                          (ibuf_buf_byp & ldst_samebeat_dc5 & ldst_dual_dc5);
   ...
   rvdffe     #(.WIDTH(pt.XLEN))         obuf_addrff       (.din(obuf_addr_in[pt.XLEN-1:0]),   .dout(obuf_addr),       .en(obuf_wr_en),                                                                                           .*);
   rvdffe     #(.WIDTH(2*pt.XLEN))       obuf_dataff       (.din(obuf_data_in[(2*pt.XLEN)-1:0]), .dout(obuf_data),     .en(obuf_wr_en),                                                                                           .*);
   ...
   for (genvar i=0; i<DEPTH; i++) begin
      ...
      assign buf_sz_in[i]         = ibuf_drainvec_vld[i] ? ibuf_sz : ({lsu_pkt_dc5.word, lsu_pkt_dc5.half} | ({2{(pt.XLEN == 64) & lsu_pkt_dc5.dword}}));
      ...
      always_comb begin
         ...
         case (buf_state[i])
            IDLE: begin
                     ...
                     buf_data_in[i]   = (ibuf_drain_vld & (i == ibuf_tag)) ? ibuf_data_out[pt.XLEN-1:0] : store_data_lo_dc5[pt.XLEN-1:0];
   ...
   always_comb begin
      ...
      lsu_nonblock_load_data_lo[pt.XLEN-1:0] = '0;
      lsu_nonblock_load_data_hi[pt.XLEN-1:0] = '0;
      for (int i=0; i<DEPTH; i++) begin
          ...
          lsu_nonblock_load_data_lo[pt.XLEN-1:0]     |= buf_data[i][pt.XLEN-1:0] & {pt.XLEN{(~buf_write[i] & (buf_state[i] == DONE) & (~buf_dual[i] | ~buf_dualhi[i]))}};
          lsu_nonblock_load_data_hi[pt.XLEN-1:0]     |= buf_data[i][pt.XLEN-1:0] & {pt.XLEN{(~buf_write[i] & (buf_state[i] == DONE) & (buf_dual[i] & buf_dualhi[i]))}};
      end
   end
   ...
   assign lsu_nonblock_data_unalgn[pt.XLEN-1:0] = 32'({lsu_nonblock_load_data_hi[pt.XLEN-1:0], lsu_nonblock_load_data_lo[pt.XLEN-1:0]} >> 8*lsu_nonblock_addr_offset[1:0]);

   assign lsu_nonblock_load_data_valid = lsu_nonblock_load_data_ready & ~lsu_nonblock_load_data_error;
   always_comb begin
      case (lsu_nonblock_sz)
         2'b00: lsu_nonblock_load_data = lsu_nonblock_unsign ? {{pt.XLEN-8{1'b0}},                         lsu_nonblock_data_unalgn[7:0]} :
                                                               {{pt.XLEN-8{lsu_nonblock_data_unalgn[7]}},  lsu_nonblock_data_unalgn[7:0]};
         2'b01: lsu_nonblock_load_data = lsu_nonblock_unsign ? {{pt.XLEN-16{1'b0}},                        lsu_nonblock_data_unalgn[15:0]} :
                                                               {{pt.XLEN-16{lsu_nonblock_data_unalgn[15]}},lsu_nonblock_data_unalgn[15:0]};
         2'b10: lsu_nonblock_load_data = lsu_nonblock_unsign ? {{pt.XLEN-32{1'b0}},                        lsu_nonblock_data_unalgn[31:0]} :
                                                               {{pt.XLEN-32{lsu_nonblock_data_unalgn[31]}},lsu_nonblock_data_unalgn[31:0]};
         2'b11: lsu_nonblock_load_data = lsu_nonblock_data_unalgn;
         default: lsu_nonblock_load_data = '0;  // Unreachable
      endcase
   end
   ...
   assign lsu_imprecise_error_addr_any[pt.XLEN-1:0] = lsu_imprecise_error_store_any ? buf_addr[lsu_imprecise_error_store_tag] : buf_addr[lsu_nonblock_load_data_tag];
