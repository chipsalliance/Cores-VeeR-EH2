...
`include "eh2_param.vh"
   ...
   input logic [pt.XLEN-1:0]            lsu_addr_dc2,                     // lsu address flowing down the pipe
   input logic [pt.XLEN-1:0]            lsu_addr_dc3,                     // lsu address flowing down the pipe
   input logic [pt.XLEN-1:0]            lsu_addr_dc4,                     // lsu address flowing down the pipe
   input logic [pt.XLEN-1:0]            lsu_addr_dc5,                     // lsu address flowing down the pipe

   input logic [pt.XLEN-1:0]            end_addr_dc2,                     // lsu address flowing down the pipe
   input logic [pt.XLEN-1:0]            end_addr_dc3,                     // lsu address flowing down the pipe
   input logic [pt.XLEN-1:0]            end_addr_dc4,                     // lsu address flowing down the pipe
   input logic [pt.XLEN-1:0]            end_addr_dc5,                     // lsu address flowing down the pipe

   input logic [(pt.XLEN*2)-1:0]        store_data_ext_dc3,               // store data flowing down the pipe
   input logic [(pt.XLEN*2)-1:0]        store_data_ext_dc4,               // store data flowing down the pipe
   input logic [(pt.XLEN*2)-1:0]        store_data_ext_dc5,               // store data flowing down the pipe
   ...
   output logic [pt.XLEN-1:0]           bus_read_data_dc3,                   // the bus return data
   ...
   output logic [pt.NUM_THREADS-1:0][pt.XLEN-1:0]   lsu_imprecise_error_addr_any,     // address of the imprecise error
   ...
   input  logic [(pt.XLEN*2)-1:0]       lsu_axi_rdata,
   ...
   logic [pt.XLEN-1:0]       store_data_lo_dc3, store_data_lo_dc4, store_data_lo_dc5;
   ...
   logic [pt.XLEN-1:0]    ld_fwddata_dc2, ld_fwddata_dc3;
   ...
   logic [(pt.XLEN*2)-1:pt.XLEN]     ld_fwddata_dc2_nc;
   ...
   logic [(pt.XLEN*2)-1:0]              bus_rsp_rdata;

   logic                                bus_rsp_valid_q, bus_rsp_ready_q, bus_rsp_write_q, bus_rsp_error_q;
   logic                                bus_rsp_write_tid_q;
   logic [(pt.XLEN*2)-1:0]              bus_rsp_rdata_q;

   logic [pt.NUM_THREADS-1:0] tid_bus_buffer_pend_any;

   logic [pt.NUM_THREADS-1:0][pt.XLEN-1:0] tid_imprecise_error_addr_any;     // address of the imprecise error
   ...
   logic [pt.NUM_THREADS-1:0][pt.XLEN-1:0]                        tid_nonblock_load_data;
   ...
   logic [(pt.XLEN*2)-1:0] lsu_axi_rdata_q;
   ...
   for (genvar i=0; i<pt.XLEN_BYTES; i++) begin
      ...
      assign ld_fwddata_lo[8*i+:8] = ld_byte_dc3hit_lo[i]    ? ld_fwddata_dc3pipe_lo[8*i+:8] :
                                     ld_byte_dc4hit_lo[i]    ? ld_fwddata_dc4pipe_lo[8*i+:8] :
                                     ld_byte_dc5hit_lo[i]    ? ld_fwddata_dc5pipe_lo[8*i+:8] :
                                                               ld_fwddata_buf_lo[lsu_pkt_dc2.tid][8*i+:8];

      assign ld_fwddata_hi[8*i+:8] = ld_byte_dc3hit_hi[i]    ? ld_fwddata_dc3pipe_hi[8*i+:8] :
                                     ld_byte_dc4hit_hi[i]    ? ld_fwddata_dc4pipe_hi[8*i+:8] :
                                     ld_byte_dc5hit_hi[i]    ? ld_fwddata_dc5pipe_hi[8*i+:8] :
                                                               ld_fwddata_buf_hi[lsu_pkt_dc2.tid][8*i+:8];
   ...
   assign bus_rsp_rdata[(pt.XLEN*2)-1:0]     = lsu_axi_rdata[(pt.XLEN*2)-1:0];
   ...
   assign lsu_axi_awaddr[pt.XLEN-1:0]   = obuf_sideeffect[bus_tid] ? obuf_addr[bus_tid][pt.XLEN-1:0] : {obuf_addr[bus_tid][pt.XLEN-1:3],3'b0};
   assign lsu_axi_awsize[2:0]           = obuf_sideeffect[bus_tid] ? {1'b0, obuf_sz[bus_tid][1:0]} : (pt.XLEN == 64) ? 3'b100 : 3'b011;
   ...
   assign lsu_axi_araddr[pt.XLEN-1:0]   = obuf_sideeffect[bus_tid] ? obuf_addr[bus_tid][pt.XLEN-1:0] : {obuf_addr[bus_tid][pt.XLEN-1:3],3'b0};
   assign lsu_axi_arsize[2:0]           = obuf_sideeffect[bus_tid] ? {1'b0, obuf_sz[bus_tid][1:0]} : (pt.XLEN == 64) ? 3'b100 : 3'b011;
   ...
   rvdffe      #(.WIDTH(pt.XLEN*2))      lsu_axi_rdata_ff   (.din(lsu_axi_rdata[(pt.XLEN*2)-1:0]), .dout(lsu_axi_rdata_q[(pt.XLEN*2)-1:0]), .en((lsu_axi_rvalid | clk_override) & lsu_bus_clk_en), .*);
   ...
   always_comb begin
      ...
      for (int i=0; i<pt.NUM_THREADS; i++) begin
         ...
         lsu_nonblock_load_data       |= {pt.XLEN{lsu_nonblock_load_data_tid == i}} & tid_nonblock_load_data[i];
      end
   end
   ...
   // Fifo flops
   rvdffe #(.WIDTH(pt.XLEN)) lsu_fwddata_dc3ff (.din(ld_fwddata_dc2[pt.XLEN-1:0]), .dout(ld_fwddata_dc3[pt.XLEN-1:0]), .en((lsu_pkt_dc2.valid & lsu_pkt_dc2.load & lsu_busreq_dc2) | clk_override), .*);
...
`ifdef RV_ASSERT_ON
     ...
     assert_lsu_axi_awaddr_aligned: assert property (lsu_axi_awaddr_aligned) else
       $display("Assertion lsu_axi_awaddr_aligned failed: lsu_axi_awvalid=1'b%b, lsu_axi_awsize=3'h%h, lsu_axi_awaddr=%d'h%h",lsu_axi_awvalid, lsu_axi_awsize[2:0], pt.XLEN, lsu_axi_awaddr[pt.XLEN-1:0]);
     ...
     assert_lsu_axi_araddr_aligned: assert property (lsu_axi_araddr_aligned) else
       $display("Assertion lsu_axi_araddr_aligned failed: lsu_axi_awvalid=1'b%b, lsu_axi_awsize=3'h%h, lsu_axi_araddr=%d'h%h",lsu_axi_awvalid, lsu_axi_awsize[2:0], pt.XLEN, lsu_axi_araddr[pt.XLEN-1:0]);
    ...
    property lsu_axi_awvalid_stable;
     ...
     property lsu_axi_awaddr_stable;
        @(posedge clk) disable iff(~rst_l)  (lsu_axi_awvalid & (lsu_axi_awaddr[pt.XLEN-1:0] != $past(lsu_axi_awaddr[pt.XLEN-1:0]))) |-> $past(lsu_bus_clk_en);
     endproperty
     ...
     property lsu_axi_wdata_stable;
        @(posedge clk) disable iff(~rst_l)  (lsu_axi_wvalid & (lsu_axi_wdata[(pt.XLEN*2)-1:0] != $past(lsu_axi_wdata[(pt.XLEN*2)-1:0]))) |-> $past(lsu_bus_clk_en);
     endproperty
     ...
     property lsu_axi_araddr_stable;
        @(posedge clk) disable iff(~rst_l)  (lsu_axi_arvalid & (lsu_axi_araddr[pt.XLEN-1:0] != $past(lsu_axi_araddr[pt.XLEN-1:0]))) |-> $past(lsu_bus_clk_en);
     endproperty
