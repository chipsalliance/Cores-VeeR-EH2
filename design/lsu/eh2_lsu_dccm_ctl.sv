...
`include "eh2_param.vh"
  ...
  (
   ...
   input logic [(pt.XLEN*2)-1:0]           dma_mem_wdata,
   input logic [2:0]                       dma_mem_tag_dc3,

   input logic [pt.XLEN-1:0]               lsu_addr_dc1,              // starting byte address for loads
   input logic [pt.XLEN-1:0]               lsu_addr_dc2,              // starting byte address for loads
   input logic [pt.XLEN-1:0]               lsu_addr_dc3,              // starting byte address for loads
   input logic [pt.XLEN-1:0]               lsu_addr_dc4,              // starting byte address for loads
   input logic [pt.XLEN-1:0]               lsu_addr_dc5,              // starting byte address for loads

   input logic [pt.XLEN-1:0]               end_addr_dc1,
   ...
   input logic [pt.XLEN-1:0]              picm_fwd_data_dc2,
   ...
   output logic [(pt.XLEN*2)-1:0]         store_data_ext_dc3, store_data_ext_dc4, store_data_ext_dc5,   // goes to the stbuf/busbuf for load-store fwdding
   ...
   output logic [pt.XLEN-1:0]             picm_mask_data_dc3,        // pic data to stbuf
   output logic [pt.XLEN-1:0]             picm_rd_data_dc3,          // pic read data in dc3
   ...
   output logic [(pt.XLEN*2)-1:0]         dccm_dma_rdata,            // dccm data to dma request
   ...
   output logic [pt.XLEN-1:0]              picm_rdaddr,        // address for pic access - shared between reads and write
   output logic [pt.XLEN-1:0]              picm_wraddr,        // address for pic access - shared between reads and write
   output logic [pt.XLEN-1:0]              picm_wr_data,       // write data
   input logic [pt.XLEN-1:0]               picm_rd_data,       // read data
   ...
   logic [(pt.XLEN*2)-1:0]            dccm_dout_dc3, dccm_corr_dout_dc3;
   logic [(pt.XLEN*2)-1:0]            stbuf_fwddata_dc3;
   logic [(pt.DCCM_BYTE_WIDTH*2)-1:0] stbuf_fwdbyteen_dc3;
   logic [(pt.XLEN*2)-1:0]            lsu_rdata_dc3, lsu_rdata_corr_dc3;
   logic [pt.XLEN-1:0]                picm_rd_data_dc2;
   logic [pt.XLEN-1:0]                picm_rd_dataQ;
   logic [(pt.XLEN*2)-1:pt.XLEN]      lsu_dccm_data_dc3_nc, lsu_dccm_data_corr_dc3_nc;
   ...
   logic [(pt.DCCM_BYTE_WIDTH*2)-1:0]   ldst_byteen_dc2, ldst_byteen_dc3, ldst_byteen_dc4, ldst_byteen_dc5;
   logic [(pt.DCCM_BYTE_WIDTH*2)-1:0]   ldst_byteen_ext_dc2, ldst_byteen_ext_dc3, ldst_byteen_ext_dc4, ldst_byteen_ext_dc5;
   logic [pt.XLEN-1:0]  store_data_hi_dc3, store_data_lo_dc3;
   ...
   assign dccm_dma_rdata[(pt.XLEN*2)-1:0] = addr_in_pic_dc3 ? {2{picm_rd_data_dc3[pt.XLEN-1:0]}} : ldst_dual_dc3 ? lsu_rdata_corr_dc3[(pt.XLEN*2)-1:0] : {2{lsu_rdata_corr_dc3[pt.XLEN-1:0]}};

   assign {lsu_dccm_data_dc3_nc[(pt.XLEN*2)-1:pt.XLEN], lsu_dccm_data_dc3[pt.XLEN-1:0]} = lsu_rdata_dc3[(pt.XLEN*2)-1:0] >> 8*lsu_addr_dc3[1:0];
   assign {lsu_dccm_data_corr_dc3_nc[(pt.XLEN*2)-1:pt.XLEN], lsu_dccm_data_corr_dc3[pt.XLEN-1:0]} = lsu_rdata_corr_dc3[(pt.XLEN*2)-1:0] >> 8*lsu_addr_dc3[1:0];

   assign dccm_dout_dc3[(pt.XLEN*2)-1:0]      = {dccm_data_hi_dc3[pt.DCCM_DATA_WIDTH-1:0], dccm_data_lo_dc3[pt.DCCM_DATA_WIDTH-1:0]};
   assign dccm_corr_dout_dc3[(pt.XLEN*2)-1:0] = {sec_data_hi_dc3[pt.DCCM_DATA_WIDTH-1:0], sec_data_lo_dc3[pt.DCCM_DATA_WIDTH-1:0]};
   assign stbuf_fwddata_dc3[(pt.XLEN*2)-1:0]  = {stbuf_fwddata_hi_dc3[pt.DCCM_DATA_WIDTH-1:0], stbuf_fwddata_lo_dc3[pt.DCCM_DATA_WIDTH-1:0]};
   ...
   for (genvar i=0; i<pt.DCCM_BYTE_WIDTH*2; i++) begin: GenLoop
      assign lsu_rdata_dc3[8*i+:8] = stbuf_fwdbyteen_dc3[i] ? stbuf_fwddata_dc3[8*i+:8] : dccm_dout_dc3[8*i+:8];
      assign lsu_rdata_corr_dc3[8*i+:8] = stbuf_fwdbyteen_dc3[i] ? stbuf_fwddata_dc3[8*i+:8] : dccm_corr_dout_dc3[8*i+:8];
   end
   ...
   // DCCM outputs
   assign ldst_byteen_dc2[(pt.DCCM_BYTE_WIDTH*2)-1:0] = ({pt.DCCM_BYTE_WIDTH*2{lsu_pkt_dc2.by}}    & (pt.DCCM_BYTE_WIDTH*2)'('b0000_0001)) |
                                                        ({pt.DCCM_BYTE_WIDTH*2{lsu_pkt_dc2.half}}  & (pt.DCCM_BYTE_WIDTH*2)'('b0000_0011)) |
                                                        ({pt.DCCM_BYTE_WIDTH*2{lsu_pkt_dc2.word}}  & (pt.DCCM_BYTE_WIDTH*2)'('b0000_1111)) |
                                                        ({pt.DCCM_BYTE_WIDTH*2{lsu_pkt_dc2.dword}} & (pt.DCCM_BYTE_WIDTH*2)'('b1111_1111)) |
  ...
  assign ldst_byteen_dc5[(pt.DCCM_BYTE_WIDTH*2)-1:0] =  ({pt.DCCM_BYTE_WIDTH*2{lsu_pkt_dc5.by}}    & (pt.DCCM_BYTE_WIDTH*2)'('b0000_0001)) |
   ...
   assign ldst_byteen_ext_dc2[(pt.DCCM_BYTE_WIDTH*2)-1:0] = ldst_byteen_dc2[(pt.DCCM_BYTE_WIDTH*2)-1:0] << lsu_addr_dc2[$clog2(pt.DCCM_BYTE_WIDTH)-1:0];      // The packet in dc2
   ...
   assign dccm_wr_bypass_c1_c2_hi   = (stbuf_addr_any[pt.DCCM_BITS-1:2] == end_addr_dc2[pt.DCCM_BITS-1:2]) & addr_in_dccm_dc2 & ~lsu_pkt_dc2.sc;   // SC always aligned and upper XLEN bits are used for ECC corrected data

   assign dccm_wr_bypass_c1_c3_lo   = (stbuf_addr_any[pt.DCCM_BITS-1:2] == lsu_addr_dc3[pt.DCCM_BITS-1:2]) & addr_in_dccm_dc3;
   assign dccm_wr_bypass_c1_c3_hi   = (stbuf_addr_any[pt.DCCM_BITS-1:2] == end_addr_dc3[pt.DCCM_BITS-1:2]) & addr_in_dccm_dc3 & ~lsu_pkt_dc3.sc;   // SC always aligned and upper XLEN bits are used for ECC corrected data

   assign dccm_wr_bypass_c1_c4_lo   = (stbuf_addr_any[pt.DCCM_BITS-1:2] == lsu_addr_dc4[pt.DCCM_BITS-1:2]) & addr_in_dccm_dc4;
   assign dccm_wr_bypass_c1_c4_hi   = (stbuf_addr_any[pt.DCCM_BITS-1:2] == end_addr_dc4[pt.DCCM_BITS-1:2]) & addr_in_dccm_dc4 & ~lsu_pkt_dc4.sc;   // SC always aligned and upper XLEN bits are used for ECC corrected data

   assign dccm_wr_bypass_c1_c5_lo   = (stbuf_addr_any[pt.DCCM_BITS-1:2] == lsu_addr_dc5[pt.DCCM_BITS-1:2]) & addr_in_dccm_dc5;
   assign dccm_wr_bypass_c1_c5_hi   = (stbuf_addr_any[pt.DCCM_BITS-1:2] == end_addr_dc5[pt.DCCM_BITS-1:2]) & addr_in_dccm_dc5 & ~lsu_pkt_dc5.sc;   // SC always aligned and upper XLEN bits are used for ECC corrected data

   // For SC conditional, hi data is used for ecc corrected data since in case of sc fail we just need to write corrected data.
   assign store_data_lo_dc3[pt.XLEN-1:0]= (lsu_pkt_dc3.atomic & ~lsu_pkt_dc3.lr & ~lsu_pkt_dc3.sc) ? amo_data_dc3[pt.XLEN-1:0] : store_ecc_data_lo_dc3[pt.XLEN-1:0];
   assign store_data_hi_dc3[pt.XLEN-1:0]= (lsu_pkt_dc3.atomic & ~lsu_pkt_dc3.lr & ~lsu_pkt_dc3.sc) ? amo_data_dc3[pt.XLEN-1:0] : (lsu_pkt_dc3.atomic & lsu_pkt_dc3.sc) ? sec_data_lo_dc3[pt.XLEN-1:0] : store_ecc_data_hi_dc3[pt.XLEN-1:0];
   ...
   if (pt.LOAD_TO_USE_PLUS1 == 1) begin: GenL2U_1
      ...
      logic [pt.XLEN-1:0] stbuf_data_any_Q;

      for (genvar i=0; i<pt.DCCM_BYTE_WIDTH; i++) begin: Gen_dccm_data
         assign dccm_data_lo_dc3[8*i+:8]     = dccm_rd_data_lo[8*i+:8];
         assign dccm_data_hi_dc3[8*i+:8]     = dccm_rd_data_hi[8*i+:8];

         assign dccm_data_lo_dc4_in[8*i+:8]  = (lsu_stbuf_commit_any &  dccm_wr_bypass_c1_c3_lo & ~ldst_byteen_ext_dc3[i])    ? stbuf_data_any[8*i+:8] :
                                               (lsu_stbuf_commit_any_Q & dccm_wr_bypass_c1_c2_lo_Q & ~ldst_byteen_ext_dc3[i]) ? stbuf_data_any_Q[8*i+:8] : store_data_lo_dc3[8*i+:8];
         assign dccm_data_hi_dc4_in[8*i+:8]  = (lsu_stbuf_commit_any &  dccm_wr_bypass_c1_c3_hi & ~ldst_byteen_ext_dc3[i+4])    ? stbuf_data_any[8*i+:8] :
                                               (lsu_stbuf_commit_any_Q & dccm_wr_bypass_c1_c2_hi_Q & ~ldst_byteen_ext_dc3[i+4]) ? stbuf_data_any_Q[8*i+:8] : store_data_hi_dc3[8*i+:8];
      end
      ...
      rvdffe #(pt.XLEN) stbuf_data_anyff (.din(stbuf_data_any[pt.XLEN-1:0]), .dout(stbuf_data_any_Q[pt.XLEN-1:0]), .en(lsu_stbuf_commit_any | clk_override), .*);
      ...
      for (genvar i=0; i<pt.DCCM_BYTE_WIDTH; i++) begin: Gen_dccm_data
         assign dccm_data_lo_dc2[8*i+:8]     = (lsu_stbuf_commit_any &  lsu_pkt_dc2.store & dccm_wr_bypass_c1_c2_lo & ~ldst_byteen_ext_dc2[i])   ? stbuf_data_any[8*i+:8] : dccm_rd_data_lo[8*i+:8]; // for ld choose dccm_out
         assign dccm_data_hi_dc2[8*i+:8]     = (lsu_stbuf_commit_any &  lsu_pkt_dc2.store & dccm_wr_bypass_c1_c2_hi & ~ldst_byteen_ext_dc2[i+4]) ? stbuf_data_any[8*i+:8] : dccm_rd_data_hi[8*i+:8]; // for ld this is used for ecc

         assign dccm_data_lo_dc4_in[8*i+:8]  = (lsu_stbuf_commit_any &  dccm_wr_bypass_c1_c3_lo & ~ldst_byteen_ext_dc3[i])   ? stbuf_data_any[8*i+:8] : store_data_lo_dc3[8*i+:8];
         assign dccm_data_hi_dc4_in[8*i+:8]  = (lsu_stbuf_commit_any &  dccm_wr_bypass_c1_c3_hi & ~ldst_byteen_ext_dc3[i+4]) ? stbuf_data_any[8*i+:8] : store_data_hi_dc3[8*i+:8];
      end
   ...
   for (genvar i=0; i<pt.DCCM_BYTE_WIDTH; i++) begin: Gen_dccm_data_dc4_dc5
      assign dccm_data_lo_dc5_in[8*i+:8]  = (lsu_stbuf_commit_any &  dccm_wr_bypass_c1_c4_lo & ~ldst_byteen_ext_dc4[i])   ? stbuf_data_any[8*i+:8] : store_data_lo_dc4[8*i+:8];
      assign dccm_data_hi_dc5_in[8*i+:8]  = (lsu_stbuf_commit_any &  dccm_wr_bypass_c1_c4_hi & ~ldst_byteen_ext_dc4[i+4]) ? stbuf_data_any[8*i+:8] : store_data_hi_dc4[8*i+:8];

      // This is store buffer write data
      assign dccm_datafn_lo_dc5[8*i+:8]   = (lsu_stbuf_commit_any &  dccm_wr_bypass_c1_c5_lo & ~ldst_byteen_ext_dc5[i]) ? stbuf_data_any[8*i+:8] :
                                                                                 (lsu_pkt_dc5.atomic & lsu_pkt_dc5.sc & ~lsu_sc_success_dc5) ? store_data_hi_dc5[8*i+:8] : store_data_lo_dc5[8*i+:8];
      assign dccm_datafn_hi_dc5[8*i+:8]   = (lsu_stbuf_commit_any &  dccm_wr_bypass_c1_c5_hi & ~ldst_byteen_ext_dc5[i+4]) ? stbuf_data_any[8*i+:8] : store_data_hi_dc5[8*i+:8];
   ...
   // PIC signals. PIC ignores the lower 2 bits of address since PIC memory registers are XLEN-bits
   ...
   assign picm_rdaddr[pt.XLEN-1:0]  = lsu_addr_dc1[pt.XLEN-1:0];
   assign picm_wraddr[pt.XLEN-1:0]  = dma_pic_wen ? dma_mem_addr[pt.XLEN-1:0] : lsu_addr_dc5[pt.XLEN-1:0];
   assign picm_wr_data[pt.XLEN-1:0] = dma_pic_wen ? dma_mem_wdata[pt.XLEN-1:0] : store_data_lo_dc5[pt.XLEN-1:0];

   // getting raw store data back for bus
   assign store_data_ext_dc3[(pt.XLEN*2)-1:0] = {store_ecc_data_hi_dc3[pt.XLEN-1:0], store_ecc_data_lo_dc3[pt.XLEN-1:0]};   // We don't need AMO here since this is used for fwding and there can't be a load behind AMO
   assign store_data_ext_dc4[(pt.XLEN*2)-1:0] = {store_data_hi_dc4[pt.XLEN-1:0], store_data_lo_dc4[pt.XLEN-1:0]};
   assign store_data_ext_dc5[(pt.XLEN*2)-1:0] = {store_data_hi_dc5[pt.XLEN-1:0], store_data_lo_dc5[pt.XLEN-1:0]};

   // Flops
   assign picm_mask_data_dc3[pt.XLEN-1:0] = picm_rd_dataQ[pt.XLEN-1:0];   // Can't use picm_rd_data_dc3 since we don't need forward data here (this is mask)
   assign picm_rd_data_dc2 = picm_fwd_en_dc2 ? picm_fwd_data_dc2[pt.XLEN-1:0] : picm_rd_data[pt.XLEN-1:0];
   rvdff #(pt.XLEN) picm_data_ff    (.*, .din(picm_rd_data_dc2[pt.XLEN-1:0]), .dout(picm_rd_data_dc3[pt.XLEN-1:0]), .clk(lsu_pic_c1_dc3_clk));
   rvdff #(pt.XLEN) picm_rd_data_ff (.*, .din(picm_rd_data[pt.XLEN-1:0]),     .dout(picm_rd_dataQ[pt.XLEN-1:0]),    .clk(lsu_pic_c1_dc3_clk));
